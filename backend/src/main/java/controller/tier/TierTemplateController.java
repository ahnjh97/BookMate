...
public class TierTemplateController extends HttpServlet {
      ...
      Object data =
          id == null
              ? service.findTemplates(
                  request.getParameter("keyword"), false, memberId(request), bookId)
              : service.findTemplate(Long.parseLong(id));
      gson.toJson(
          Map.of("success", true, id == null ? "templates" : "template", data),
          response.getWriter());
    ...
    try {
      ...
      long id =
          service.requestTemplate(
              memberId, body.title, body.description, body.category, body.bookIds);
      response.setStatus(201);
      gson.toJson(
          Map.of("success", true, "templateId", id, "message", "템플릿이 관리자 검토 대기열에 등록되었습니다."),
          response.getWriter());
  ...
  private Long memberId(HttpServletRequest request) {
    ...
    Object id = s == null ? null : s.getAttribute("loginMemberId");
  ...
  private void error(HttpServletResponse r, int status, String message) throws IOException {
    r.setStatus(status);
    gson.toJson(
        Map.of("success", false, "message", message == null ? "요청을 처리하지 못했습니다." : message),
        r.getWriter());
  }
