...
@WebServlet("/api/posts/comments/create")
public class PostCommentCreateController extends HttpServlet {
    ...
    protected void doPost(
        ...
        try {
            commentRequest = readRequestBody(request);
        ...
        try {
            long commentId = postCommentService.createComment(
    ...
    /* 2. 요청 JSON 읽기 */
    private CreateCommentRequest readRequestBody(
            HttpServletRequest request
    ) throws IOException {
        request.setCharacterEncoding("UTF-8");

        try (BufferedReader reader = request.getReader()) {
            return gson.fromJson(reader, CreateCommentRequest.class);
        }
    }
