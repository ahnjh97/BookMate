...
async function load() {
  try {
    const r = await fetch(`/api/ideal/templates?id=${id}`), d = await r.json();
    if (!r.ok) throw new Error(d.message);
    template = d.template;
    title.textContent = template.title;
    desc.textContent = template.description || "더 마음에 드는 책을 선택하세요.";
    document.querySelector("#stats-link").href = `/pages/ideal/stats.html?id=${id}`;
...
async function save() {
  versus.innerHTML = "<p class=\"ideal-status\">결과를 저장하는 중입니다.</p>";
  try {
    const r = await fetch("/api/ideal/runs", {
        method: "POST",
      ...
      d = await r.json();
    if (!r.ok) throw new Error(d.message);
    location.href = `/pages/ideal/result.html?id=${d.runId}`;
  } catch (e) {
    versus.innerHTML = `<p class="ideal-status">${esc(e.message)}</p>`;
  }
}
function card(b, i) {
