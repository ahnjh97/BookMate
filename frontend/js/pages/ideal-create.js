...
function render() {
  ...
  books.filter(b => !q || b.title.toLowerCase().includes(q) || (b.authorName || "").toLowerCase().includes(q)).forEach(
    b => {
      ...
      l.className = "ideal-book-option";
      l.innerHTML = `${
        b.imageUrl ? `<img src="${esc(b.imageUrl)}" alt="">` : "<i class=\"cover-placeholder\"></i>"
...
document.querySelector("#form").onsubmit = async e => {
  ...
  try {
    const r = await fetch("/api/ideal/templates", {
        method: "POST",
    ...
    location.href = `/pages/ideal/play.html?id=${d.templateId}`;
  } catch (x) {
