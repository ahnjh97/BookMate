...
function Get-ExpectedHash {
    ...
    try {
        ...
        $match = [regex]::Match($checksumText, '(?i)\b[0-9a-f]{128}\b')
...
try {
    ...
    elseif ($env:M2_HOME -and (Test-Path (Join-Path $env:M2_HOME 'bin\mvn.cmd'))) {
    ...
    if ($configuredTomcat -and (Test-Path (Join-Path $configuredTomcat 'bin\catalina.bat'))) {
    ...
    Write-Host '[1/3] Building the BookMate WAR file...'
    & $MavenExecutable -f (Join-Path $ProjectRoot 'backend\pom.xml') package
    if ($LASTEXITCODE -ne 0) {
        throw "Maven build failed with exit code $LASTEXITCODE."
    }

    Write-Host '[2/3] Deploying the WAR file to Tomcat...'
    $warPath = Join-Path $ProjectRoot 'backend\target\bookmate.war'
    $deployedWar = Join-Path $TomcatRoot 'webapps\bookmate.war'
